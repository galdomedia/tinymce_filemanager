  module TinymceFilemanager
    ...
    def make_image_thumb(base_folder, file, navi_list)
      image = MiniMagick::Image.open(save_directory(base_folder, validate_name(file.original_filename), navi_list)).first
      image = image.resize "#{@@thumbs_width}x#{@@thumbs_height}#"
